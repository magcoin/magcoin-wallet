...
public final class BitcoinIntegration
{
	...
	{
		final StringBuilder uri = new StringBuilder("magcoin:");
		if (address != null)
			uri.append(address);
		if (amount != null)
	...
	{
		Toast.makeText(context, "No MagCoin application found.\nPlease install MagCoin Wallet.", Toast.LENGTH_LONG).show();

		final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=de.schildbach.wallet.magcoin"));
		final Intent binaryIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://github.com/magcoin/magcoin-wallet/releases"));
